...
MedicalEmployee::MedicalEmployee() : degree(""), specialization(""),
	Employee("","","", ContactInformation(),"",0,"",0,0)
{
}

MedicalEmployee::MedicalEmployee(std::string name, std::string dateOfBirth, std::string emergencyContact, ContactInformation &contactInfo, std::string employeeID, int clockedIn, std::string salary, int hoursWorkedPerWeek, int experience, std::string degree, std::string specialization) :
...
bool MedicalEmployee::hasPatient(const Patient patient)
{
	for (Patient p : patients)
	{
		if (p.getPatientID() == patient.getPatientID()) // if we find the patient, return the patient.
			return true;
	}
	return false;
}
